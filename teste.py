...
def visualizar():
    ...
    # === LÓGICA PARA POLÍTICAS DE OPENCDN ===
        ...
        # OPENCDN sempre usa o nome fixo, não precisa de listagem
        policy_name_cliente = OPENCDN_BASE_NAME # Força o nome para ser usado no get_policy_details
...
def configurar():
    ...
    if not net_connect:
        ...
        if not mensagem: # Se não houver erro preliminar
            if sentido == "IN":
                novo_lp_str = request.form.get('novo_lp')
                try:
                    novo_lp = int(novo_lp_str)
                    if 0 <= novo_lp <= 4294967295:
                        mensagem, _ = configurar_local_preference(net_connect, policy_name_config, node_alvo, novo_lp)
                    else:
                        mensagem = "❌ Local-Preference fora do range válido."
                except ValueError:
                    mensagem = "❌ LP inválido. Digite um número inteiro."

            elif sentido == "OUT":
                as_path_str = request.form.get('novo_as_path')
                mensagem, _ = configurar_as_path_prepend(net_connect, policy_name_config, node_alvo, as_path_str)
    ...
    return redirect(url_for('visualizar', 
                            ...
                            codigo_cliente=codigo if tipo_politica == 'OPERADORA' else '',
                            # Usa policy_name_cliente_redirect para Cliente e OPENCDN, pois contém o nome da política base
                            policy_name_cliente=policy_name_cliente_redirect if tipo_politica in ['CLIENTE', 'OPENCDN'] else '',
